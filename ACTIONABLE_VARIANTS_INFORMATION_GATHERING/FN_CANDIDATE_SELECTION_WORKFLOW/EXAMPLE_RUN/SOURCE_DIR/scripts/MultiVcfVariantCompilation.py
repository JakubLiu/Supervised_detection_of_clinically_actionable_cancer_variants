...
def MultiVcfVariantCompilation(vcf_list_path, output_path):
    ...
    # Process each VCF (gzipped or plain)
    for file_idx, path in enumerate(vcf_paths, start=1):
        # pysam handles both .vcf and .vcf.gz automatically
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Compile variants across multiple VCF files (supports .vcf and .vcf.gz).")
