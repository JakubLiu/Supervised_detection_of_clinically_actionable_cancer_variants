...
def process_pileup_FN_call(raw_pileup_file_path, output_file_path):

    cutoffs = snakemake.config.get('cutoffs', [])

    with open(raw_pileup_file_path, "r", encoding='utf-8') as infile, \
        open(output_file_path, "w", encoding="utf-8") as outfile:

        header = 'CHROM,POS,REF,SAMPLE,COVERAGE,READ_BASES,N_ALT_BASES,' + ','.join(['FN_at_cutoff_'+str(cutoff) for cutoff in cutoffs]) + '\n'
        ...
        for line in infile:  # here one line has multiple samples
            ...
            for sample in lines_splitted:    # here one line is one sample
                ...
                cutoff_pass = ['NO']*len(cutoffs)

                if coverage > 0:
                    prop_alt_reads = n_alternative_bases/coverage

                    for c in range(0, len(cutoffs)):
                        current_cutoff = cutoffs[c]

                        if prop_alt_reads >= current_cutoff:
                            cutoff_pass[c] = 'YES'
                

                output_line = sample[:-1] + [translated_sequence, n_alternative_bases] + cutoff_pass
...
process_pileup_FN_call(raw_pileup_file_path=snakemake.input[0],
