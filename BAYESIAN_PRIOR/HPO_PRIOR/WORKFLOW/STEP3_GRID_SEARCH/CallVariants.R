...
CallVariants <- function(bayes_factor,   # an R object returned by the function MakeBayesFactor()
                        ...
                        vcf <- deepSNV:::bf2Vcf(
                                ...
                                sample = c(tumor_bam, normal_cohort),
                                prior = prior_value,
                                mvcf = TRUE
                                )
                        ...
                        called_loci <- data.frame(
                                ...
                                ref = as.character(ref(called)),
                                alt = as.character(unlist(alt(called)))
                        )
                        ...
                        if (output_file_path != FALSE){
                                ...
                                write.csv(called_loci, output_file_path, row.names=FALSE)
                        }

                        writeVcf(vcf, output_vcf_path)
