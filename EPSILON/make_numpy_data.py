...
data_samples = pd.read_csv('data_negative_controls.csv')
data_reads = pd.read_csv('per_read_data.csv')
...
for i in range(0, data_samples.shape[0]):
    ...
    '''
    # If you want to use 1D 4 channel CNN's instead of attention, then you need one hot encoding
    one_hot_seq = np.zeros((len(raw_genomic_context), len(['A', 'T', 'G', 'C', 'N'])), dtype=np.float32)

    for j, base in enumerate(raw_genomic_context):
        idx = base_to_int.get(base, 4)  # if not found default to 4 = N
        one_hot_seq[j, idx] = 1.0

    x_free_genomic.append(one_hot_seq)
    '''
...
np.savez("training_data_chr_pos.npz",
...
np.savez("validation_data_chr_pos.npz",
