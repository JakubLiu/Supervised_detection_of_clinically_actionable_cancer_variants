...
class TransformerEncoder(tf.keras.layers.Layer):
    ...
    def __init__(self, base_embedding_dim, dense_dim, num_heads, output_dim, **kwargs):
        super().__init__(**kwargs)
    ...
    def call(self,x):
        ...
        projected_multi_head_self_attention_block_output = self.projection(multi_head_self_attention_block_output)  # project to a specific dimensionality (must be the same as the dimensionality of the other 3 components)
        
        return projected_multi_head_self_attention_block_output
...
class InfoSourceSharing(tf.keras.Model):
    def __init__(self, component_dim, num_heads, num_components = 4):
          super().__init__()

          self.mask = ~tf.eye(num_components, dtype = tf.bool)
          self.MaskedMHSA = tf.keras.layers.MultiHeadAttention(num_heads = num_heads, key_dim = component_dim // num_heads)  # masked MHSA
    ...
    def call(self, x, training = True):
         ...
         # masked MHSA
         y = self.MaskedMHSA(
              query=x, key=x, value=x, attention_mask=self.mask
         )

         # flatten to a tensor of shape (batch_size, 4*component_dim)
         y = tf.reshape(y, (-1, 4 * self.component_dim))
         return y
...
class FinalProjection(tf.keras.Model):
    def __init__(self, hidden_dim1, hidden_dim2, hidden_activation_fun = 'relu', dropout_rate = 0.0):
        ...
        self.MLP = tf.keras.Sequential([
            ...
            tf.keras.layers.Dropout(dropout_rate),
            tf.keras.layers.Dense(2, activation = 'sigmoid')  # one estimator for the probability of being zero, and the other node for the binomial probability parameter
        ])
...
class Epsilon2(tf.keras.Model):
    def __init__(self,
                 
                 components_output_dim,   # the output dimension of all the 4 input components
                ...
                self._config = dict(
                     components_output_dim=components_output_dim,
                    intructive_genomic_input_dim=intructive_genomic_input_dim,
                    instructive_genomic_hidden_dim1=instructive_genomic_hidden_dim1,
                    instructive_genomic_hidden_dim2=instructive_genomic_hidden_dim2,
                    instructive_sample_input_dim=instructive_sample_input_dim,
                    instructive_sample_hidden_dim1=instructive_sample_hidden_dim1,
                    instructive_sample_hidden_dim2=instructive_sample_hidden_dim2,
                    free_sample_hidden_dim1_psi=free_sample_hidden_dim1_psi,
                    free_sample_hidden_dim2_psi=free_sample_hidden_dim2_psi,
                    free_sample_output_dim_psi=free_sample_output_dim_psi,
                    free_sample_hidden_dim1_phi=free_sample_hidden_dim1_phi,
                    free_sample_hidden_dim2_phi=free_sample_hidden_dim2_phi,
                    genomic_context_size=genomic_context_size,
                    num_unique_bases=num_unique_bases,
                    base_embedding_dim=base_embedding_dim,
                    dense_dim=dense_dim,
                    num_heads=num_heads,
                    final_proj_dim1=final_proj_dim1,
                    final_proj_dim2=final_proj_dim2,
                    instructive_genomic_activation_fun=instructive_genomic_activation_fun,
                    intructive_genomic_dropout_rate=intructive_genomic_dropout_rate,
                    instructive_sample_activation_fun=instructive_sample_activation_fun,
                    instructive_sample_dropout_rate=instructive_sample_dropout_rate,
                    free_sample_dropout_rate_psi=free_sample_dropout_rate_psi,
                    free_sample_activation_fun_psi=free_sample_activation_fun_psi,
                    free_sample_aggregation_function=free_sample_aggregation_function,
                    free_sample_dropout_rate_phi=free_sample_dropout_rate_phi,
                    free_sample_activation_fun_phi=free_sample_activation_fun_phi,
                    final_proj_dropout_rate=final_proj_dropout_rate,
                    final_projection_hidden_activation_fun=final_projection_hidden_activation_fun,
                )
                ...
                self.InstructiveGenomic = InstructiveGenomic(
                     ...
                     output_dim = components_output_dim,
                ...
                self.FreeSample = FreeSample(
                     ...
                     output_dim_phi = components_output_dim,
                ...
                self.information_source_sharing = InfoSourceSharing(
                     component_dim=components_output_dim,
                     num_heads=num_heads,
                     num_components=4
                )
    ...
    def call(self,
         ...
         stacked = tf.stack([free_sample_output, instructive_sample_output, free_genomic_context_output, intructive_genomic_output], axis=1)
         flattened_post_masked_MHSA = self.information_source_sharing(stacked)
         ZIB_estimates = self.FinalProjection(flattened_post_masked_MHSA, training=training)

         return ZIB_estimates
