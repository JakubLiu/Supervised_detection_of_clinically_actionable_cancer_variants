...
class Epsilon(tf.keras.Model):
    def __init__(self,
                ...
                self._config = dict(
                    ...
                    instructive_genomic_output_dim=instructive_genomic_output_dim,
                    ...
                    instructive_sample_output_dim=instructive_sample_output_dim,
                    ...
                    free_sample_output_dim_phi=free_sample_output_dim_phi,
...
class BinomialNegLogLoss(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        """
        Here I cast the y's to a higher precision, because if I use a lower precision then it gets
        rounded to 0 and log(0) --> -Inf ---> NaN down the road
        """
...
class ZeroInfBinomialLoss(tf.keras.losses.Loss):
    ...
    def __init__(self, pseudocount=1e-8, normalize_by_coverage=False, name="binomial_nll", **kwargs):
    ...
    def get_config(self):
        config = super().get_config()
        config.update({
            "pseudocount": self.eps,
            "normalize_by_coverage": self.normalize,
        })
        return config
