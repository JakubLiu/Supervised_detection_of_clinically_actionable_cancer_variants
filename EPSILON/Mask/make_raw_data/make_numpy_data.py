...
data_samples = pd.read_csv('per_locus_smalldata.txt')
data_reads = pd.read_csv('per_read_smalldata.csv')
...
# the two following will not be used during training
chrom_list = []
pos_list = []
...
for i in range(0, data_samples.shape[0]):
    ...
    current_chrom = str(data_samples.iloc[i,0])
    ...
    raw_genomic_context = list(data_samples.iloc[i,-1])  # extract only a 21bp window around the locus of interest
    x_free_genomic.append([base_to_int[x] for x in raw_genomic_context])

    chrom_list.append(current_chrom)
    pos_list.append(current_pos)



    x_instructive_genomic.append([float(x) for x in list(data_samples.loc[i, ["GGC_upstream","CGG_downstream","GGT_upstream","TGG_downstream","GC_content"]])])  # remove some homo feature due to almonst no variance
    x_instructive_sample.append([float(x) for x in list(data_samples.loc[i, ["median_mapQ","median_baseQ", "mismatches"]])])
    k_true.append(float(data_samples.loc[i,'general_alt_counts']))  # alt counts
...
#x_free_genomic = np.stack(x_free_genomic, axis = 0)   # use this instead of the below if you want to use CNN's
x_free_genomic = np.array(x_free_genomic, dtype = np.float64)
...
chrom_array = np.array(chrom_list)
pos_array = np.array(pos_list)


train_mask = np.random.uniform(0,1, y_true.shape[0]) < 0.7
...
# things not used during training
chrom_train = chrom_array[train_mask]
chrom_val   = chrom_array[~train_mask]

pos_train = pos_array[train_mask]
pos_val   = pos_array[~train_mask]


np.savez("training_data_chr_pos.balans.npz",
...
np.savez("validation_data_chr_pos.balans.npz",
