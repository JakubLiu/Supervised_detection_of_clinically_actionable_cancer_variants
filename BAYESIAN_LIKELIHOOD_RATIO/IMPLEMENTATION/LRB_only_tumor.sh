...
if [[ "$DISTINGUISH_STRANDS" == "NO" ]]; then

  Rscript LRB_tumor_only_one_direction.R \
                      "$TUMOR_BAM" \
                      "$NEGATIVE_CONTROL_BAMLIST" \
                      "$CHROMOSOME" \
                      "$START" \
  ...
  echo "wrong option for --distinguish_strands, choose from "YES" or "NO" (default)"
