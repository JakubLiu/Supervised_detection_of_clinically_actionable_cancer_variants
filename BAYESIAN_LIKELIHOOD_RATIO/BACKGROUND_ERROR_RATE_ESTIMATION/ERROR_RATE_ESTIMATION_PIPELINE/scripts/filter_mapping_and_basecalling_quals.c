/*
This programme masks bases that have a basecalling quality below a user defined threshold as 'N'.
It also skips the reads that have a mapping quality below a user defined threshold.
*/
...
int main(int argc, char *argv[]) {
    ...
    int Q_THRESHOLD = atoi(argv[3]);   // minimum basecalling quality
    int MAP_QUAL_THRESHOLD = atoi(argv[4]);    // minimum mapping quality
    ...
    while (sam_read1(in, hdr, b) >= 0) {   // loop over the records in the bam file
        ...
        if (b->core.flag & BAM_FUNMAP) // skip unmapped reads
            continue;

        
        if (b->core.qual == 255 || b->core.qual < MAP_QUAL_THRESHOLD) // skip reads that have a too low mapping quality (255 means that the mapping quality is not available)
            continue;
